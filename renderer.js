...
async function renderDesktop() {
  ...
  items.forEach((it, idx) => {
    ...
    // başlangıçta varsayılan glyph kullan
    icon.innerHTML = `<div class="glyph">${glyph(it)}</div><div class="label">${it.name}</div>`;

    // eğer .lnk dosyası ise hedefe göre glyph'i güncelle
    const ext = (it.name.match(/\.[^.]+$/) || [])[0] || '';
    if (ext === '.lnk') {
      const glyphEl = icon.querySelector('.glyph');
      const linkPath = jn('/Desktop', it.name);
      resolveLink(linkPath).then(act => {
        // Eğer uygulama kısayoluysa, uygulama isminin ilk harfi veya bir emoji kullan
        if (act.action === 'launch-app') {
          const m = (typeof appManifests !== 'undefined') ? appManifests.find(x => x.id === act.appId) : null;
          glyphEl.textContent = m?.emoji || (m?.name ? m.name.charAt(0) : '🧩');
        } else if (act.action === 'open-folder') {
          glyphEl.textContent = '📁';
        } else if (act.action === 'open-file') {
          glyphEl.textContent = '📄';
        } else {
          glyphEl.textContent = '🔗';
        }
  ...
  desk.onkeydown = async (e) => {
    ...
    if (e.key === 'Delete') {
      ...
      for (const n of selNames) {
        await window.vfs.delete('/Desktop', n, { toRecycle: true }).catch(err => window.ui.showAlert({ title: 'Hata', message: err.message }));
        delete layout[n];
      }
      await saveDesktopLayout(layout).catch(() => { });
